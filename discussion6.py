...
###############################################################################
    def load_results(self, table):
        ...
        '''
        #first row of table is the header row
        header = table[0]
        race_dict = {}

        for row in table[1:]:
            #name of the horse is the inner key 
            horse_name = row[0]
            #dict of races is the inner value 
            horse_races = {}

            #iterate through the races only
            for i in range(len(row[1:])): 
                horse_races[header[i+1]] = float(row[i+1])

            #add the horse's times to the dict 
...
###############################################################################
    ...
    def horse_fastest_race(self, horse):
        ...
        #get the list of races of that horse
